...
import {
  ...
  balloonCloseButtonClickHandler,
  markerButtonClickHandler,
} from "./utils/_handlers.js";
...
async function initMap(mapElement, isBaseMap, pins, settings) {
  ...
  if (!apiKey) {
    throw new Error(
      "API ключ для Yandex Maps не найден. Убедись, что он передан в HTML.",
    );
  }
  ...
  nearButton &&
    nearButton.addEventListener("click", () => getGeolocation(pins));
  ...
  allCloseButtons &&
    allCloseButtons.forEach((button) => {
      button.addEventListener("click", () =>
        balloonCloseButtonClickHandler(button),
      );
    });
  ...
  allMarkerButtons &&
    allMarkerButtons.forEach((button) => {
